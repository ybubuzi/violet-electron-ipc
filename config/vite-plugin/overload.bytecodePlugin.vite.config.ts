...
export function bytecodePlugin(options: BytecodeOptions = {}): Plugin | null {
  ...
  return {
    ...
    async writeBundle(options, _output): Promise<void> {
      if (options.format === 'es' || useInRenderer || !bytecodeRequired) {
        return;
      }

      const outDir = options.dir!;
      const output: typeof _output = {};
      bytecodeFiles = [];
      for (const [key, value] of Object.entries(_output)) {
        if (!(value.name && excludeList.includes(value.name))) {
          output[key] = value;
        }
      }
